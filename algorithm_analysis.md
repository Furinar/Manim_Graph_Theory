...
- **无权图场景**：
  ...
  - **结果**：路径长度为 3 (边数)。
  - **分析**：BFS 保证找到边数最少的路径。在本图中，S->A->C->T 和 S->B->D->T 都是 3 条边，BFS 找到了其中之一，符合预期。

- **有权图场景**：
  - **结果**：路径总权重 3+2+4 = **9**。
  - **实际最短路径**：S -> B -> D -> T，总权重 1+2+3 = **6**。
  - **分析**：**BFS 失败**。
    1.  BFS 仅按层级扩展，完全忽略边的权重。
    2.  它认为 S->A (权重 3) 和 S->B (权重 1) 代价相同（都是 1 跳）。
    3.  由于邻居排序 `A < B`，BFS 优先探索了 A 分支，导致最终选定了一条高成本路径。
...
| **A\***      | 启发式(预估) | Priority Queue | 高效路径规划 | **优点**: 效率高, 保证最短路(h 可采纳)<br>**缺点**: 内存大, 启发函数难设计         |
